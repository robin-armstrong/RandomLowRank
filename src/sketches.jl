...
An abstract type whose subtypes represent different procedures for matrix sketching. For any concrete type `ConcreteSketch <: Sketch`, the method
	...
	sketch(A::Matrix, l::Integer, side::String, sk::ConcreteSketch, rng::AbstractRNG)
	
must be implemented. This method returns the sketch of `A` according to the procedure that `ConcreteSketch` specifies, in such a way that the column dimension is preserved (for `side == "right"`) or the row dimension is preserved (for `side == "left"`), with all internal randomness being generated by `rng`. The non-preserved dimension of the returned matrix must be at least `l`.
"""
...
function sketch(A::Matrix, l::Integer, side::String, sk::NoSketch, rng::AbstractRNG)
	return deepcopy(A)
end
...
function sketch(A::Matrix, l::Integer, side::String, sk::GaussianSketch, rng::AbstractRNG)
	if(l < 1)
...
function sketch(A::Matrix, l::Integer, side::String, sk::HadamardSketch, rng::AbstractRNG)
	if(l < 1)
		throw(SketchError("target dimension of sketch must be positive"))
	elseif(side == "left")
		if(l > size(A, 1))
		...
		for i = 1:hadamardDim
			presketch[i, :] *= rand(rng) < .5 ? hadamardDim : -hadamardDim
		end

		presketch = fwht(presketch, 1)	# Walsh-Hadamard transform on the columns
		rowindices = broadcast(x -> ceil(Int64, hadamardDim*x), rand(rng, l))
	...
	elseif(side == "right")
		...
		colindices = broadcast(x -> ceil(Int64, hadamardDim*x), rand(rng, l))
...
function sketch(A::Matrix, l::Integer, side::String, sk::CWSketch, rng::AbstractRNG)
	if(l < 1)
		throw(SketchError("target dimension of sketch must be positive"))
	elseif(side == "left")
		if(l > size(A, 1))
		...
		signs = rand(rng, Bool, size(A, 1))
		row_unused = ones(Bool, size(A, 1))
		sketchedmat = zeros(l, size(A, 2))

		for i = 1:l
			for r = 1:size(A, 1)
				if(row_unused[r] && ((i == l) || (rand(rng) < 1/(l - i + 1))))
	...
	elseif(side == "right")
		...
		signs = rand(rng, Bool, size(A, 2))
		col_unused = ones(Bool, size(A, 2))
		sketchedmat = zeros(size(A, 1), l)

		for i = 1:l
			for c = 1:size(A, 2)
				if(col_unused[c] && ((i == l) || (rand(rng) < 1/(l - i + 1))))
